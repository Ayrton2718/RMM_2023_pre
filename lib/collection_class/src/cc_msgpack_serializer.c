#include "cc_msgpack_serializer.h"
...
{
    OPDumper_t dumper = OPMsgpackDumper_create();
...
{
    OPLoader_t loader = OPMsgpackDumper_create();
...
static CC_obj CCMsgpackSerializer_objectMaker(OPLoader_t loader, OPType_token_t token)
{
    CC_obj obj;
    ...
    case OPType_objectType_DICTIONARY:
        {
            ...
            for(size_t dic_i = 0; dic_i < count; dic_i++)
            {
                ...
                CC_obj cur_obj = CCMsgpackSerializer_objectMaker(loader, cur_token);
...
{
    CC_obj obj = NULL;
    OPLoader_t loader = OPMsgpackLoader_create();
    if(OPLoader_load(loader, (const uint8_t*)CCBinary_getRaw(bin_obj), CCBinary_size(bin_obj)))
    {
        obj = CCMsgpackSerializer_objectMaker(loader, OPLoader_getRoot(loader));
    }
    OPLoader_destructor(loader);
